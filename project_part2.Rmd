...
Outlier detection and standardization. (Not sure which dataset we need to use here...Shall we impute the missing data first or shall we deal with outliers first?) So I use the original dataset for now:)

```{r}
# For "area" and "rooms", we can find that some observations with a very small area per room, implying that there might be measurement error in "area" and "rooms". Hence, we replace these outliers with NA.
# summary(train_data$roomarea) 
# summary(train_data$area) 
# summary(train_data$rooms) 
train_data$roomarea <- train_data$area/train_data$rooms
train_data$area <- replace(train_data$area, train_data$roomarea < 5|train_data$roomarea > 200, NA)
train_data$rooms <- replace(train_data$rooms, train_data$roomarea < 5|train_data$roomarea > 200, NA)
...
# What if we create a new variable, "age"? We find that some are built in the future, 2021 or 2020...
train_data$building_age <- 2020 - train_data$year_built
...
# var format transformations (Do we need to transform data type before imputing missing data?)
